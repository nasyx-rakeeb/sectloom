...
- **Provenance**: Provenance creates cryptographic signed attestations about the build environment linking the package to its source repository. However, provenance requires a cloud-hosted CI/CD environment (like GitHub Actions with OIDC). As instructed by the project phase guidelines, we will *not* configure CI/CD merely for provenance.
